...
interface ObjectionHandlingScript {
  ...
  sourceYear?: number;
  sourceUrl: string;
  intentGroup?: 'time' | 'single-subject' | 'version' | 'price' | 'initiative' | 'service' | 'defer' | 'subject-fit' | 'short-package' | 'other';
  keywords: string[];
  content: string;
...
const OBJECTION_HANDLING_SCRIPTS: ObjectionHandlingScript[] = [
  ...
  },
  {
    ...
    keywords: ['短期', '小课包', '短期课包', '先报短期', '报短期', '只买短期', '短期同步课', '之前报过短期课', '没怎么看过', '怕后面不愿意学', '新鲜感'],
    content:
...
function extractQueryTerms(query: string): string[] {
  ...
  const shortPackageTerms =
    /短期|小课包|小课|短期课包|先报短期|报短期|买短期|短期同步课|新鲜感/.test(normalized)
      ? ['短期', '小课包', '短期课包', '先报短期', '只买短期', '短期同步课', '之前报过短期课', '新鲜感']
      : [];
  return [...new Set([
    ...baseTerms,
    ...splitTerms,
...
function inferObjectionIntentGroup(query: string): ObjectionHandlingScript['intentGroup'] | null {
  ...
  if (/短期|小课包|小课|短期课包|先报短期|报短期|买短期|短期同步课|新鲜感/.test(normalized)) return 'short-package';
