...
function extractQueryTerms(query: string): string[] {
  ...
  const liberalArtsTerms =
    /文科|政史地|历史|政治|地理|亏|不划算/.test(normalized)
      ...
      : [];
  const deferTerms =
    /先看看|先试试|先体验|后面.*再说|以后再说|有需要再说|需要再说|等开学|不着急|考虑|商量|问爸爸|问妈妈|问老公|问孩子|问老师|爸爸不同意|妈妈不同意|父母不同意|孩子.*决定|老师.*看看/.test(normalized)
      ? ['孩子先看看后面有需要再说', '现在不着急', '先看看', '后面有需要再说', '有需要再说', '以后再说', '等开学再说', '再考虑一下', '商量', '问爸爸', '问妈妈', '问老公', '问孩子', '问老师']
      : [];
  const priceTerms =
...
function inferObjectionIntentGroup(query: string): ObjectionHandlingScript['intentGroup'] | null {
  ...
  if (/先看看|先试试|先体验|后面.*再说|以后再说|有需要再说|需要再说|等开学|不着急|考虑|商量|问爸爸|问妈妈|问老公|问孩子|问老师|爸爸不同意|妈妈不同意|父母不同意|孩子.*决定|老师.*看看/.test(normalized)) return 'defer';
