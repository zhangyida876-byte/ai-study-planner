...
- 每个已填科目必须点名当前年级和学期正在学习的至少2个模块，写出家长可观察现象、根因、后续影响和验证方法；详细执行只在报告第6节输出，并区分未来7天、未来1个月和当前学期。禁止只写“培养习惯、打好基础”。
...
- 个体诊断只分析已填分数科目；未填科目只能作为页面上的共性学情参考，不得参与个体档位、目标差距或问题判断。
- 跨学科影响必须按“底层能力→影响科目→影响机制→家长表现→验证方式”输出，只能使用结构化教研资料中真实、可验证的关联；无可靠关联就不输出，禁止为了数量硬凑。
- 语文阅读可影响应用题审题、理科题干信息提取和文科材料阅读，但不得被写成影响数学/物理符号运算；英语长难句主要影响英语学科内部阅读、完形和续写，不强行关联数学。
- 全报告的详细家长动作只能出现在第6节，三个周期分别承担逐日执行、每周节奏和学期目标，禁止复制同一套内容。
