...
9. 本模块必须完整承载共性专业资料：当前进度、学期目标、重难点、易错点、卡点、家长现象、后续影响、可靠跨科关联、年龄心理和洋葱课程匹配；不得挪用个体诊断中的成绩区间、目标差距或销售推进结论。
10. 跨学科影响必须写清“底层能力→影响科目→影响机制→可观察表现”。语文阅读可影响应用题审题、理科题干提取和政史地材料阅读，但不得写成影响数学/物理符号运算；没有可靠关联就不输出。
...
【诊断专项硬性要求】
1. 必须严格使用7节模板结构和标题，先给顾问可直接照读的诊断总结话术，再按已填科目输出具体原因与现象依据、关键节点、量化危机链、三周期行动、产品承接和可复制话术。
1.1 7个章节必须全部返回；信息不足时保留标题并写“需补充信息后进一步判断”，不得省略任何章节。
1.2 第6节必须独立使用“## 6. 洋葱执行计划”，第7节必须独立使用“## 7. 洋葱学园承接方案 + 可复制话术”；禁止改名或因篇幅原因截掉。
1.3 第1节必须独立使用“## 1. 顾问先讲：诊断总结”，写成300-500字、可直接发送或照读的完整口语总结；它必须由第2至第7节浓缩而来，不得只报分数和等级。总结必须讲清得分率、下一个关键节点、预计暴露分值/成绩风险区间、当天作业耗时、单科或多科压力、孩子信心、未来7天动作和洋葱承接。
...
6. 每个已填写科目回答：当前分/满分/得分率、水平等级、单科2个或多科每科1-2个关键根因，以及对应的关键节点、量化风险和动作。
...
15. 任何行动建议必须先服从“当前教学进度硬约束”。当当前日期处于开学前或开学初时，第6节只能安排已学内容、前置知识和即将进入的第一单元；不得从整学期目录随意抽取函数综合、实验大题、压轴题等后续内容。
16. 第3节每个问题必须按“核心问题、背后根因、家长怎么验证”三行输出，禁止使用竖线压成一行。
17. 第4节必须点名下一个关键节点、当前进度知识点、预计暴露分值、成绩风险区间和判断依据；生地会考、实验考试、模考、学考、选科等差异节点必须标注待当地或学校核实。
18. 第5节必须输出“当前漏洞→当天作业→近期考试→其他科目/总分→信心与节奏→目标差距”的量化危机链，数字只能取自系统量化参考或用户输入。
19. 第6节必须区分未来7天、未来1个月、当前学期，并增加6.4“专属学习规划与协同跟进”：前三个周期分别输出3-4个近期动作、最多4周节奏、最多3个学期阶段；6.4写清顾问定方向、课程与练习解决问题、助教按实际服务周期（通常每周）协助跟进、家长配合检查和调整边界，禁止暗示每日人工陪学。
20. 第7节先输出7.1洋葱承接短表及专属学习规划说明，每个已填科目必须同时呈现同步打底和分层培优：报告价值表达约40%讲同步、约60%重点讲培优的考点、题型、变式、步骤得分、应试策略和试卷库验证；该比例不得解释为固定课时或卷面占分。再输出7.2-7.6五类独立话术，并以7.7输出可搭配素材。普通话术每条80-180字，风险提醒150-260字。每个推荐功能必须解释解决什么、为什么有效、怎么使用、如何与前后功能组合以及家长如何验收，禁止只罗列名称。
20.1 若长度受限，先压缩第2-5节的专业说明；第1节、6.4、7.1、7.2-7.7属于销售必用内容，禁止被截断、合并或省略。
20.0 第7.2-7.6节同时是电话沟通5步，必须严格按“共鸣开场、问诊定位、风险讲透、洋葱承接、推进下一步”形成连续可照读链路；第2-6节及第7.1节保持短行或表格结构，供微信看板直接截图。
20.1 所有分数段都必须评估培优路径：低分段使用基础题型、常考点和得分步骤培优，中等段使用中档题、变式和应试策略培优，中上段使用综合模型、限时策略和稳定性培优。必须先通过同步基础验证，再按层级升级，禁止低分直接做高难题。
20.2 小学、初中、高中必须使用不同的培优重点：小学聚焦方法、应用与阶段测查；初中聚焦月考/中考题型、考点和步骤得分；高中聚焦学考/高考模型、规范表达、限时策略和选科相关能力。
...
34. 顾问负责整体诊断、升学目标和阶段方向，课程与练习负责解决具体问题，助教只按实际服务范围协助制定周计划并做周期性跟进，家长配合简短检查；不会题必须先定位知识点、题型和错因，再用对应课程解决一类题。学习阻抗场景从5-8分钟短课和可完成题低阻启动。第7.5话术必须让家长听懂这一服务闭环且不得承诺每日人工陪学，第7.7只推荐适合当前沟通的2-3类素材并保留动态信息核实边界。

【本模块边界】
0. 下面的模块边界和建议输出结构优先级高于插件默认模板；如默认模板要求输出长篇政策/完整规划，应主动压缩或省略。
1. 本模块只服务当前具体孩子，第1节先把“目前什么水平、家长看到了什么、真正卡在哪里、下个节点预计丢多少、作业多耗多久、未来7天做什么、洋葱如何承接”浓缩成顾问可直接照读的总结；后续章节只负责补充依据和执行明细。
2. 只引用当前目标学校或必要的本地层级参照，不展开多套学校梯度、志愿批次、选科专业和长期政策科普。
3. 必须把分数差距翻译成家长听得懂且可验证的风险区间；数字只能来自用户输入或系统量化参考，并说明估算边界，不得编造学校真实占分或录取概率。
4. 只在第6节输出行动方案：7天用于立即执行、1个月用于形成周节奏、当前学期用于阶段目标；不得在第3、第4或第5节重复行动表。
5. 输出优先采用当地课程顾问口吻：先说结论和各科问题，再给分科行动与产品承接。
6. 除第1节外，第一至六节优先使用短结论、分行标签和表格，不得连续输出超过120字的大段文字；第7节输出产品承接和顾问可复制话术。
7. 同一份报告会同时生成“电话话术”和“微信学情看板”：第1节与第7.2-7.6节优先满足电话直接照读；第2-6节与第7.1节必须使用短行、分科结构和表格，便于微信页面截图，不得把多个维度挤成一段。

【问题定位证据规则】
1. 分数只能用于提出“待验证的问题假设”，不能仅凭单次分数断言具体知识点已经失分。
2. 单科列2个、多科每科列1-2个最高优先级问题，并在“家长怎么验证”中说明用哪道错题、哪次作业或哪类小测确认。
3. 危机链必须对应已发现的问题、当天作业表现、后续章节、下一考试节点、孩子信心和目标差距；多科时还要说明复习时间与总分稳定性的叠加压力，不得用恐吓式结论。
4. 第3节的各科根因必须与第5节危机、第6节行动和第7.1节产品方案逐科对应；第7.2-7.6节用5类顾问话术完成沟通推进。
