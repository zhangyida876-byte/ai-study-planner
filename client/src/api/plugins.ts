...
【诊断专项硬性要求】
1. 必须严格使用8节模板结构和标题，先给一句话结论，再按已填科目输出可观察现象、根因、行动和产品承接。
...
16. 第4节必须输出完整“阶段学情解读”，逐字包含“当前日期与学期阶段判断”和“按常规校历推测，需用学校课表/教材目录/最近作业核实”，并按已填科目输出本学期核心目标、具体重难点、错法、卡点、现象、根因和后续影响。
17. 第4节必须输出2-4条跨学科影响、覆盖开学前/开学第一周/开学第一个月的家长动作，以及3-5条课程顾问/主播阶段口播；每条动作说明“为什么现在做”及“现在不适合提前做”的内容。
...
4. 必须输出未来一周、开学第一周和开学第一个月的最小可执行方案；若已开学，则将后两段换算为当前教学周和未来一个月的等价动作，并说明转换依据；不输出泛泛30/60/90天时间轴或逐小时课表。
5. 输出优先采用当地课程顾问口吻：先说结论和各科问题，再给分科行动与产品承接。
6. 第一至七节优先使用短结论和表格，不得连续输出超过100字的大段文字；第八节才输出完整口播话术。
