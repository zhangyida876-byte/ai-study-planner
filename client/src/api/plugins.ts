...
19. 第6节必须区分未来7天、未来1个月、当前学期，并增加6.4“专属学习规划与助教跟进方案”：前三个周期分别输出3-4个近期动作、最多4周节奏、最多3个学期阶段；6.4写清顾问与助教分工、周一至周日安排、不会题处理、督学提醒、反馈和调整，禁止复制同一套内容。
20. 第7节先输出7.1洋葱承接短表及专属学习规划与助教跟进说明，每个已填科目至少包含同步打底和分层培优两条路径；再输出7.2-7.6五类独立话术，并以7.7输出可搭配素材。普通话术每条80-180字，风险提醒150-260字。每个推荐功能必须解释解决什么、为什么有效、怎么使用、如何与前后功能组合以及家长如何验收，禁止只罗列名称。
...
34. 顾问负责整体诊断、升学目标和阶段方向，助教负责按真实可学习时间拆到每天并跟进完成；不会题必须先定位知识点、题型和错因，再用对应课程解决一类题。学习阻抗场景从5-8分钟短课和可完成题低阻启动。第7.5话术必须让家长听懂这一服务闭环，第7.7只推荐适合当前沟通的2-3类素材并保留动态信息核实边界。
