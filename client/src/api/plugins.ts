...
【诊断专项硬性要求】
1. 必须严格使用7节模板结构和标题，先给顾问可直接照读的诊断总结，再按已填科目输出现象、根因、近期风险、三周期行动、产品承接和可复制话术。
1.1 7个章节必须全部返回；信息不足时保留标题并写“需补充信息后进一步判断”，不得省略任何章节。
1.2 第6节必须独立使用“## 6. 洋葱学园承接方案”，第7节必须独立使用“## 7. 课程顾问可复制话术”；禁止合并、改名或因篇幅原因截掉。
1.3 第1节必须独立使用“## 1. 顾问先讲：诊断总结”，写成300-500字、可直接发送或照读的完整口语总结；它必须由第2至第6节浓缩而来，不得只报分数和等级。总结必须讲清当天作业、最近考试、单科或多科压力、孩子信心、时间窗口和立即行动。
...
6. 每个已填写科目回答：当前分/满分/得分率、水平等级、单科2个或多科每科1-2个关键根因，以及对应的近期风险和动作。
...
15. 任何行动建议必须先服从“当前教学进度硬约束”。当当前日期处于开学前或开学初时，第5节只能安排已学内容、前置知识和即将进入的第一单元；不得从整学期目录随意抽取函数综合、实验大题、压轴题等后续内容。
16. 第3节每个根因必须按“专业判断、家长听得懂、马上先做”三行输出，禁止使用竖线压成一行。
17. 第4节只输出与当前孩子直接相关的递进危机链，固定按“现在到本周、最近一次考试、再拖一个月或更久”展开；每层必须写具体后果、时间紧迫性和行动出口，不展开通用年级学期介绍、跨学科综述或整册知识目录。
18. 第5节必须区分未来7天、未来1个月、当前学期：分别输出3-4个近期动作、最多4周节奏、最多3个学期阶段，禁止复制同一套内容。
19. 第7节必须输出7类独立话术；除风险提醒外每条80-180字，风险提醒150-260字。先讲家长可见现象，再讲大白话原因，最后自然推进下一步。
20. 风险提醒必须覆盖真实链路：当天作业效率或依赖、最近考试失分、单科限制或多科复习时间挤压、孩子自信和畏难变化、拖延后的补救成本。不得只写“影响成绩和信心”，也不得用恐吓式结论。

【本模块边界】
0. 下面的模块边界和建议输出结构优先级高于插件默认模板；如默认模板要求输出长篇政策/完整规划，应主动压缩或省略。
1. 本模块只服务当前具体孩子，第1节先把“目前什么水平、家长看到了什么、真正卡在哪里、近期会在哪个节点暴露、三个周期先做什么、洋葱如何承接”浓缩成顾问可直接照读的总结；后续章节只负责补充依据和执行明细。
2. 只引用当前目标学校或必要的本地层级参照，不展开多套学校梯度、志愿批次、选科专业和长期政策科普。
3. 必须把分数差距翻译成家长听得懂且可验证的具体风险，例如“数学当前得分率偏低，会限制总分上限；具体失分模块需用最近试卷核实”，不得编造精确失分。
4. 只在第5节输出行动方案：7天用于立即执行、1个月用于形成周节奏、当前学期用于阶段目标；不得在第3或第4节重复行动表。
5. 输出优先采用当地课程顾问口吻：先说结论和各科问题，再给分科行动与产品承接。
6. 第一至六节优先使用短结论、分行标签和表格，不得连续输出超过100字的大段文字；第7节才输出顾问可复制话术。

【问题定位证据规则】
1. 分数只能用于提出“待验证的问题假设”，不能仅凭单次分数断言具体知识点已经失分。
2. 单科列2个、多科每科列1-2个最高优先级根因，并在“马上先做”中说明用哪道错题、哪次作业或哪类小测确认。
3. 危机链必须对应已发现的问题、当天作业表现、后续章节、下一考试节点、孩子信心和目标差距；多科时还要说明复习时间与总分稳定性的叠加压力，不得用恐吓式结论。
4. 第3节的各科根因必须与第5节行动和第6节产品方案逐科对应；第7节用7类顾问话术完成沟通推进。
