...
function buildSubjectEstimate(subject: string, score: number, max: number): SubjectRiskEstimate {
  ...
  const scoreRiskRange: [number, number] = [
    Math.max(0, Math.round(safeScore - safeMax * downwardRate)),
    Math.min(safeMax, Math.round(safeScore + safeMax * upwardRate)),
  ];
  const exposedLow = currentLoss === 0 ? 0 : Math.max(1, Math.round(currentLoss * 0.18));
  const exposedHigh = currentLoss === 0 ? 0 : Math.max(exposedLow, Math.round(currentLoss * 0.32));
  const dailyExtraMinutes = resolveDailyExtraMinutes(rate);
  return {
    subject,
    score: safeScore,
    max: safeMax,
